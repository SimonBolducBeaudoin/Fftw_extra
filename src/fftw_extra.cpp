...
complex_d_1D r2c_basic(double *in, int l_fft) {
  ...
  fftw_plan plan =
      fftw_plan_dft_r2c_1d(l_fft, in, reinterpret_cast<fftw_complex *>(&out[0]),
                           FFTW_EXHAUSTIVE); // Out of place
...
complex_d_1D r2c_advanced(double *in, int l_fft, int howmany) {
  ...
  fftw_import_wisdom_from_filename("FFTW_Wisdom.dat");
  int n[] = {l_fft};
  fftw_plan plan = fftw_plan_many_dft_r2c // In place ifft with strides
      (1,                                 // rank
...
double_1D hilbert(double *in, int l_fft) {
  const complex_d mI(
      0, -1.0 / l_fft); // Minus Imaginary number with normalisation factor
  double_1D out(l_fft + 2, fftw_malloc, fftw_free);
  complex_d_1D out_complex((complex_d *)out.get_ptr(),
                           l_fft / 2 +
                               1); // Shared memory but different cast-type

  fftw_import_wisdom_from_filename("FFTW_Wisdom.dat");
  fftw_plan plan_foward = fftw_plan_dft_r2c_1d(
      l_fft, in, reinterpret_cast<fftw_complex *>(&out_complex[0]),
      FFTW_EXHAUSTIVE); // Out of place
  fftw_plan plan_backward = fftw_plan_dft_c2r_1d(
      l_fft, reinterpret_cast<fftw_complex *>(&out_complex[0]), &out[0],
      FFTW_EXHAUSTIVE); // In place
  fftw_export_wisdom_to_filename("FFTW_Wisdom.dat");
...
complex_d_1D analytic(double *in, int l_fft) {
  const complex_d mI(
      0, -1.0 / l_fft); // Minus Imaginary number with normalisation factor
  complex_d_1D tmp(
      l_fft / 2 + 1, fftw_malloc,
      fftw_free); // Real part is the in array imaginary part is Hilbert(in).
  complex_d_1D out(
      l_fft, fftw_malloc,
      fftw_free); // Real part is the in array imaginary part is Hilbert(in).
  ...
  fftw_import_wisdom_from_filename("FFTW_Wisdom.dat");
  fftw_plan plan_foward =
      fftw_plan_dft_r2c_1d(l_fft, in, reinterpret_cast<fftw_complex *>(&tmp[0]),
                           FFTW_EXHAUSTIVE); // Out of place
  ...
  fftw_plan plan_backward = fftw_plan_many_dft_c2r(
      rank, n, howmany, reinterpret_cast<fftw_complex *>(&tmp[0]), inembed,
      istride, idist, imag_ptr, onembed, ostride, odist, flags);
  ...
  fftw_execute(plan_backward); // in-place transform with stride of
                               // 2*(sizeof(double)) in outputs
  /* Copy operation for the real part */
  for (int i = 0; i < l_fft; i++) {
    *(real_ptr + 2 * i) = in[i]; // Multiplication is there so that Real and
                                 // imaginary parts have the same normalization
  }
